...
| File                                                       | Usage                                                                                                        |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `src/app.scss`                                             | Main import, `--pico-font-size` overrides (6 breakpoints)                                                    |
...
| `src/lib/components/GoogleFormField.svelte`                | `--pico-spacing`                                                                                             |
| `src/lib/components/GoogleForm.svelte`                     | `class="container"`, `<article>`                                                                             |
| `src/lib/components/GroupRegistration.svelte`              | `--pico-muted-color`, `--pico-del-color`, `--pico-color`                                                     |
| Veneer `+layout.svelte`                                    | `--pico-muted-color`, `--pico-color`, `--pico-card-sectioning-background-color`, `role="group"`, Swiper a11y |
| `[flags]` `+page.svelte`                                   | `--pico-muted-color`, `--pico-color`, `--pico-card-sectioning-background-color`, `role="group"`, Swiper a11y |
| `src/routes/+page.svelte`                                  | `class="container"`                                                                                          |
| `src/routes/(centered)/+layout.svelte`                     | `class="container"` (to be deleted)                                                                          |
...
4. **Semantic elements** - `<article>`, `role="group"` (no `role="button"` in source)
...
> **Note:** The veneer layout's inner `<content>` element no longer has a `max-width` — it fills the body width. Component-level constraints (Sheet `dl`, NotificationBox) still use `$size-content-2` where appropriate.
...
Uses `$size-content-3` (60ch) — already wider than Google Forms. The `html` element gets a distinct background color to create visual contrast on wide screens:

```scss
@use 'open-props-scss' as *;

html {
	background-color: $gray-1; // Outside content area
}
...
body {
	...
	background-color: $gray-0; // Content area
}
```

### CSS Reset

Uses [sanitize.css](https://github.com/csstools/sanitize.css) instead of a manual reset. Imported in `+layout.svelte`:

- `sanitize.css` — base reset (box-sizing, body margin, overflow-wrap, etc.)
- `sanitize.css/forms.css` — form font inheritance, select arrow
- `sanitize.css/typography.css` — orphans, widows, etc.
...
| `--pico-spacing`                          | `$size-3`                                  |
| `--pico-del-color`                        | `$red-6` / `$red-4` (dark)                 |
...
1. `AutogrowingTextarea.svelte` - Replace 4 pico vars (background, border-width, border-color, radius)
2. `NotificationBox.svelte` - Replace 2 pico vars (card-sectioning-bg, background)
3. `Sheet.svelte` - Replace 3 pico vars (spacing, card-sectioning-bg, muted-border)
4. `StickyHeaderSummaryDetailsGrid.svelte` - Replace 2 pico vars (muted-border, card-sectioning-bg)
5. `GoogleFormField.svelte` - Replace 1 pico var (spacing)
6. `GroupRegistration.svelte` - Replace 3 pico vars (muted-color, del-color, color)
7. Veneer `+layout.svelte` - Replace 3 pico vars, remove `role="group"`, remove Swiper a11y hack
8. `[flags]` `+page.svelte` - Replace 3 pico vars, remove `role="group"`, remove Swiper a11y hack

### Phase 3: Update app.scss + Global Centering

In `+layout.svelte`, add CSS imports before `app.scss`:
...
### Phase 5: Remove PicoCSS, Add sanitize.css

1. `pnpm add -D sanitize.css`
2. Remove `@picocss/pico` from `package.json`
3. `pnpm install`
4. Remove Swiper a11y disable hack (was only to prevent `role="group"` conflict with PicoCSS)
5. Test all form elements and layouts
