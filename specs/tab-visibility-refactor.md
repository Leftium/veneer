...
- `*` expands to all tabs in-place (no longer redirects; cleaner for modifier use like `*-info`)
...
| Phase 3: `?tabs=` modifier syntax         | Phase 2                               | ~30 lines parser + update existing param handling          | Done    |
...
| `src/routes/[id1=vid]/[[id2=vid]]/+layout.server.ts` | `detectSheetType()`, `computeDefaultTabs()`, `parseTabs()`, pass `sheetType` to client                          | 1-3 Done |
