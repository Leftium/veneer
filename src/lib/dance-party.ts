...
/**
 * Per-image bubble anchor height as a fraction of (baseIconHeight × scaleFactor),
 * measured upward from the feet. E.g. 0.8 means the bubble anchors at 80% of the
 * normalised dancer height. Tunable via /dancer-scales alongside DANCER_SCALES.
 */

export const BUBBLE_TOPS: Record<string, Record<number, number>> = {
	both: {
		'1': 0.705,
		'2': 0.68,
		'3': 0.56,
...
/**
 * Look up the bubble-anchor height fraction for a dancer image.
 * Returns DEFAULT_BUBBLE_TOP (0.8) for images without an explicit override.
 */
export function getBubbleTop(role: 'both' | 'lead' | 'follow', imageNum: number): number {
	return BUBBLE_TOPS[role]?.[imageNum] ?? DEFAULT_BUBBLE_TOP
}
