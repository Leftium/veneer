				...
				>
					<label for="extra-name-{member.key}" class="name-label">
						...
						{localeText(
							...
							locale,
							nameField.title,
						)}{#if bNameField.bilingualTitle}{@const k = `name-${member.key}`}<button
								type="button"
								class="lang-toggle"
					...
					{#if roleField}
						<label for=""
							...
							{localeText(
								...
								locale,
								roleField.title,
							)}{#if bRoleField?.bilingualTitle}{@const k = `role-${member.key}`}<button
									type="button"
									class="lang-toggle"
									class:toggled={extraToggles.has(k)}
									onclick={(e) => {
						...
						{#each roleField.options as option, j (option)}
							<label>
								...
								{localeText(
									...
									locale,
									option,
								)}{#if bRoleField?.bilingualOptions?.[j]}{@const k = `option-${member.key}-${j}`}<button
										type="button"
										class="lang-toggle"
										class:toggled={extraToggles.has(k)}
										onclick={(e) => {
